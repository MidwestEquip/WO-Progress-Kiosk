...
- Stock completions never require these fields.
...
Remaining (do not start until user says PASS):
- Patch 3: Auto-detect in openTcAssyEntry; mode badge + Change in entry modal
- Patch 4: Operator name at top of WO screens; bypass entry modal when mode is known
- Patch 5: Unit completion gate (unit fields required only for unit mode)
- Patch 6: Notes field visible on WO screen + notes prompt on unit completion
