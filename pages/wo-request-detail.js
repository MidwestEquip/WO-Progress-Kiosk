...
//          data, BOM subparts), part-print files, and the subpart
//          drill-in capture side. The restore/Done/Cancel/close side
//          lives in wo-request-subparts.js (500-line cap split);
//          the shared open-generation counter is
//          store.woRequestDetailGen (store-stock.js).
...
export async function openWoRequestDetail(req) {
    const gen = ++store.woRequestDetailGen.value;  // this open's generation; stale loads bail on mismatch
    ...
    const chainP = db.resolvePartCalcChain(req.part_number).then(({ data }) => {
        if (gen === store.woRequestDetailGen.value && data.links.length) store.woRequestCalcChain.value = data;
    ...
    // 1yr: parent BOM demand (rolling 12mo). Promise shared with the on-hand
    // panel below — its pathDetails carry the ancestor×multiplier data on-units needs.
    const demand1yrP = db.calculateRecursiveParentUsageDemand(req.part_number, oneYearAgo, today);
    demand1yrP.then(({ data, error }) => {
        if (gen !== store.woRequestDetailGen.value) return;    // stale: a newer detail opened
    ...
    // Native live on-hand (part_on_hand) + "on units" — qty of this part already
    // built into BOM ancestors still in stock. Rides the 1yr demand promise for
    // the ancestor paths (one on-hand batch fetch, no extra BFS). Gen-guarded;
    // loading flag cleared in finally so a rejection never strands the spinner.
    store.woRequestOnHand.value        = null;
    store.woRequestOnUnits.value       = null;
    store.woRequestOnHandLoading.value = true;
    demand1yrP
        .then(async ({ data: demand }) => {
            const paths     = demand?.pathDetails || [];
            const ancestors = [...new Set(paths.map(p => p.topParent))];
            const { data: ohMap, error } = await db.fetchOnHandForParts([norm, ...ancestors]);
            if (gen !== store.woRequestDetailGen.value) return;   // stale: a newer detail opened
            if (error) { logError('openWoRequestDetail:onHand', error, { part: req.part_number }); return; }
            store.woRequestOnHand.value = ohMap[norm] || { on_hand: null, counted_at: null };
            // On-units: counted ancestors only; uncounted ones contribute 0 and
            // are surfaced via the "(x of y counted)" note in the label.
            let units = 0;
            const countedSet = new Set(ancestors.filter(a => ohMap[a]?.counted_at != null));
            paths.forEach(p => {
                if (countedSet.has(p.topParent)) units += (ohMap[p.topParent].on_hand || 0) * p.multiplier;
            });
            store.woRequestOnUnits.value = { qty: units, counted: countedSet.size, total: ancestors.length };
        })
    ...
    db.fetchBomChildrenForParent(req.part_number).then(({ data, error }) => {
        ...
        if (subparts.length > 0) {
            ...
            calcSubpartStats(subparts)
                .then(s => { if (gen === store.woRequestDetailGen.value) store.woRequestSubpartStats.value = s; })
    ...
    try {
        ...
        if (gen !== store.woRequestDetailGen.value) return;    // stale: a newer detail opened during the await
        if (defaults) {
...
// ── Subpart inspect (drill-in), capture side ────────────────────────────────
...
// The restore side (finish/cancel/dismiss/close) lives in wo-request-subparts.js.
